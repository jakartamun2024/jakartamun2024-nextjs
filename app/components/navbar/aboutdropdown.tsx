...
export default function AboutDropdown(props: any) {
    const router = useRouter()
    const path = usePathname()

    function toAbout() {
        if (path !== "/about-us") {
            router.push("/about-us?s=0")
        } else if (
            props.aboutRef &&
            props.aboutRef.current
        ) {
            props.aboutRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
        }
    }

    function toGrand() {
        if (path !== "/about-us") {
            router.push("/about-us?s=1")
        } else if (
            props.grandRef &&
            props.grandRef.current
        ) {
            props.grandRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
        }
    }
